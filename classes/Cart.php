...
class CartCore extends ObjectModel
{
     ...
     */
      public static function krmacica4($idz)
    {
         $resultez = (int)Db::getInstance()->getValue('
				SELECT `SmanjenjeCeneProizvoda`
				FROM `'._DB_PREFIX_.'popust_duseci`
				WHERE `cartId` = '.(int)$idz);  
      ...
      public static function krmacica3($idz)
    {
       $result = Db::getInstance()->executeS('SELECT popustAktiviran FROM '._DB_PREFIX_.'popust_duseci WHERE cartId = '.(int)$idz);
     if($result[0]['popustAktiviran'] == 'da'){
       return true;
     }
     else{
         return false;
      ...
      public static function krmacica2($array)
    {
          $kokoro = [];
        $kemenerac = 0;
        ...
        $resultez = (int)Db::getInstance()->getValue('
				SELECT `cartId`
				FROM `'._DB_PREFIX_.'popust_duseci`
				WHERE `cartId` = '.(int)$array[0]['cartId']);  
				if($resultez == $array[0]['cartId']){
				    
				         foreach ($array as $value) {
            
           
                                     if($value['kategorijaProizvoda'] == 'duseci-sa-dzepicastim-jezgrom'){
                                           
                                             foreach ($array as $value2) {
                                              
                                                  if($value2['kategorijaProizvoda'] == 'nadduseci'){
                                                
                                                   echo '<h3>Popust od 5% na dušeke u kombinaciji sa naddušecima je aktiviran ('.$value['cenaProizvoda'] * 0.05.' dinara)</h3>';
                                                   
                                                   array_push($kokoro, $value['cartId'], $value['userId'], $value['cenaProizvoda'] * 0.05);
                                                   $kemenerac++;
                                                  }
                        ...
                        $query = "UPDATE `ps_popust_duseci` SET `popustAktiviran`='da' WHERE `cartId` = '".(int)$array[0]['cartId']."' ";
            Db::getInstance()->Execute($query);       
            
                          }
                          else if($kemenerac  == 0){
                        $query = "UPDATE `ps_popust_duseci` SET `popustAktiviran`='ne' WHERE `cartId` = '".(int)$array[0]['cartId']."' ";
            Db::getInstance()->Execute($query);      
                          }
     ...
     public static function aktivacica($array){
                                                                         $resulte = (int)Db::getInstance()->getValue('
                                                            				SELECT MAX(id_cart_rule)
                                                            				FROM `ps_cart_rule`'); 
    ...
    public static function kukurnica($array){
        ...
        foreach ($array as $value) { if($value['kategorijaProizvoda'] == 'kreveti') {$krevetiBr++; $krevetiKol+=(int)$value['kolicina'];} }
        foreach ($array as $value) { if($value['kategorijaProizvoda'] == 'duseci-sa-dzepicastim-jezgrom') {$duseciBr1++; $zzzz+=$value['cenaProizvoda']*(int)$value['kolicina']; $duseciBr1Kol=(int)$value['kolicina']; } }
        foreach ($array as $value) { if($value['kategorijaProizvoda'] == 'duseci-sa-zicanim-jezgrom') {$duseciBr2++; $zzzz+=$value['cenaProizvoda']*(int)$value['kolicina']; $duseciBr2Kol=(int)$value['kolicina'];} }
        foreach ($array as $value) { if($value['kategorijaProizvoda'] == 'deciji-duseci') {$duseciBr3++; $zzzz+=$value['cenaProizvoda']*(int)$value['kolicina']; $duseciBr3Kol=(int)$value['kolicina'];} }
        foreach ($array as $value) { if($value['kategorijaProizvoda'] == 'duseci-od-pene') {$duseciBr4++; $zzzz+=$value['cenaProizvoda']*(int)$value['kolicina']; $duseciBr4Kol=(int)$value['kolicina']; }}
           
       
          $duseciBr = $duseciBr1Kol + $duseciBr2Kol + $duseciBr3Kol + $duseciBr4Kol;

         
              
             if($krevetiKol>0){
                 if($duseciBr>0){
                                         if($krevetiKol>$duseciBr){
                                            $zabir = (float)($zzzz*0.05);
                                        Db::getInstance()->insert('popust_duseci_pet', array(
                    'active' => (int)(1),
                    'cart_id' => (int)$array[0]['cartId']
                    
                ));     
                $resulteeaww = Db::getInstance()->getValue('
                                                            				SELECT `active`
                                                            				FROM `ps_popust_duseci_pet`
                                                            				WHERE `cart_id` = '.(int)$array[0]['cartId']); 
                                                            				
                                                            			if((int)$resulteeaww == 1){	
                                        echo '<h3>Ostvarili ste pravo na popust od 5% na dušeke u kombinaciji sa krevetima. Cena će biti umanjena za '.$zabir.' dinara. <button class="buttoner">Aktiviraj popust</button></h3><div style="display:none;" id="qwert123">1</div>';
                                                            			}      
                                         }
                                         else if ($krevetiKol<=$duseciBr){
                                               $kkpom = $krevetiKol;
                                             $dbrpom = $duseciBr;
                                             $kkkkkkkkk = $kkpom/$dbrpom;
                                             $zabir = (float)($zzzz*0.05*$kkkkkkkkk);
                ...
                $resulteeaww = Db::getInstance()->getValue('
                                              ...
                                              echo '<h3>Ostvarili ste pravo na popust od 5% na dušeke u kombinaciji sa krevetima. Cena će biti umanjena za '.$zabir.' dinara. <button class="buttoner">Aktiviraj popust </button></h3><div style="display:none;" id="qwert123">1</div>';
                                                                  ...
                                                                  $resulteeeeer = Db::getInstance()->executeS('
                                                            			SELECT `id_cart_rule`
                                                            			FROM `ps_cart_rule`
                                                            			WHERE `id_customer` = '.(int)$array[0]['userId']);    
                                                                    
                                                                    foreach($resulteeeeer as $krme){
                                                                      
                                                                        
                                                                        $resulteeaw = Db::getInstance()->getValue('
                                                            				SELECT `name`
                                                            				FROM `ps_cart_rule_lang`
                                                            				WHERE `id_cart_rule` = '.(int)$krme['id_cart_rule'].' AND `id_lang` = "1"'); 
                                                            			
                                                            				
                                                            		
                                                       
                                                                        if($resulteeaw == 'Popust 5% na dušeke u kombinaciji sa krevetima'){
                                                                                
                                                                                    Db::getInstance()->execute('
                                                                						DELETE FROM `ps_cart_rule_lang`
                                                                						WHERE id_cart_rule = '.(int)$krme['id_cart_rule']);
                                                                					Db::getInstance()->execute('
                                                                ...
                                                                $cartRuleObj->name[Configuration::get('PS_LANG_DEFAULT')] = 'Popust 5% na dušeke u kombinaciji sa krevetima';
                                                                ...
                                                                $cartRuleObj->reduction_amount = (int)$zabir;
                                                                $cartRuleObj->cart_rule_restriction = 1;
                                                                $cartRuleObj->free_shipping = 0;
                                                                $cartRuleObj->active = 1;
                                                                $cartRuleObj->minimum_amount = 0;
                                                                $cartRuleObj->id_customer = $array[0]['userId'];
                                                                $cartRuleObj->add();
       ...
       public static function brisacica($array){
        $resulteeeeer = Db::getInstance()->executeS('
                                    			SELECT `id_cart_rule`
                                    			FROM `ps_cart_rule`
                                    			WHERE `id_customer` = '.(int)$array[0]['userId']);    
      ...
      public static function krmacica($array)
    {
        $kokoro = [];
        $kemenerac = 0;
        foreach ($array as $value) {
             ...
             if($value['kategorijaProizvoda'] == 'duseci-sa-dzepicastim-jezgrom'){
                     ...
                     foreach ($array as $value2) {
                          ...
                          if($value2['kategorijaProizvoda'] == 'nadduseci'){
                                 echo '<h3>Ostvarili ste pravo na popust od 5% na dušeke u kombinaciji sa naddušecima. Cena će biti umanjena za '.$value['cenaProizvoda'] * 0.05.' dinara na poslednjem koraku porudžbine. </h3>';
          ...
          $resulte = (int)Db::getInstance()->getValue('
           ...
           if($resulte>0){
				...
				if((float)$resultez != (float)$kokoro[2] ){
			
                    
          
                Db::getInstance()->execute('
					UPDATE `'._DB_PREFIX_.'popust_duseci`
					SET `SmanjenjeCeneProizvoda` = '.(float)$kokoro[2].',
                 ...
                 Db::getInstance()->insert('popust_duseci', array(
                    'cartId' => (int)$kokoro[0],
                    'userId' => (int)$kokoro[1],
                    'SmanjenjeCeneProizvoda' => (float)$kokoro[2],
                    'date' => date("Y-m-d h:i:sa"),
                    'popustAktiviran' => 'ne'
             ...
             $resultezz = (int)Db::getInstance()->getValue('
				SELECT cartId
				FROM `'._DB_PREFIX_.'popust_duseci`
				WHERE `cartId` = '.$array[0]['cartId']); 
             
             if($resultezz == $array[0]['cartId']){
              Db::getInstance()->execute('
						DELETE FROM `'._DB_PREFIX_.'popust_duseci`
						WHERE cartId = '.(int)$array[0]['cartId']);
             }
             else{
...
protected function dodajPopustPet($popustulja){
                                        $cartRuleObj = new CartRule();
                                        $cartRuleObj->date_from = date('Y-m-d H:i:s');
                                        $cartRuleObj->date_to = '2046-12-12 00:00:00';
                                        $cartRuleObj->name[Configuration::get('PS_LANG_DEFAULT')] = 'PopustNaDusekePet';
                                        $cartRuleObj->quantity = 1;
                                        $code = Tools::passwdGen();
                                        while (CartRule::cartRuleExists($code)) { // let's make sure there is no duplicate
                                            $code = Tools::passwdGen();
                                        ...
                                        $cartRuleObj->quantity_per_user = 1;
                                        $cartRuleObj->reduction_percent = 0;
                                        $cartRuleObj->reduction_amount = (int)$popustulja;
                                        $cartRuleObj->cart_rule_restriction = 1;
                                        $cartRuleObj->free_shipping = 0;
                                        $cartRuleObj->active = 1;
                                        $cartRuleObj->minimum_amount = 0;
                                        ...
                                        $cartRuleObj->add();
                                            ...
                                            $values = array(
                                        'tax_incl' => $cartRuleObj->getContextualValue(true),
                                        'tax_excl' => $cartRuleObj->getContextualValue(false)
                                    );
    ...
    */
    public function getOrderTotal($with_taxes = true, $type = Cart::BOTH, $products = null, $id_carrier = null, $use_cache = true)
    {
        ...
        if ($type == Cart::ONLY_DISCOUNTS) {
           ...
           if($this->krmacetina($virtual_context->cart)){
               $resulter = Db::getInstance()->executeS('SELECT SmanjenjeCeneProizvoda FROM '._DB_PREFIX_.'popust_duseci WHERE cartId = '.(int)$virtual_context->cart->id);
            return Tools::ps_round((float)$order_total - $resulter[0]['SmanjenjeCeneProizvoda'], $compute_precision);
            
           }
        else{  return Tools::ps_round((float)$order_total, $compute_precision);}
       // return Tools::ps_round((float)$order_total, $compute_precision);
      
    }
    protected function krmacetina($id_order){
   
         $result = Db::getInstance()->executeS('SELECT cartId,popustAktiviran FROM '._DB_PREFIX_.'popust_duseci WHERE cartId = '.(int)$id_order->id);
        
         if(!empty($result)){
        if($result[0]['cartId'] == (int)$id_order->id){
            if($result[0]['popustAktiviran'] == 'da'){
                return true;
            }
        }else{
        ...
        //$id_cart = Cart::getCartIdByOrderId($id_order);
