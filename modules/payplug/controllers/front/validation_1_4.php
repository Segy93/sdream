...
addLog($debug, $log, 'validation_1_4', 'debug');
$payplug = new Payplug();
...
/*
if (!$payplug->active) {
    addLog($debug, $log, 'Payplug module is not enabled.', 'error');
    Payplug::redirectForVersion($redirect_url_error);
} elseif (!($cart_id = Tools::getValue('cartid'))) {
*/
if (!($cart_id = Tools::getValue('cartid'))) {
    addLog($debug, $log, 'No Cart ID.', 'error');
    Payplug::redirectForVersion($redirect_url_error);
} elseif (!($ps = Tools::getValue('ps')) || $ps != 1) {
    if ($ps == 2) {
        addLog($debug, $log, 'Order has been cancelled on PayPlug page', 'info');
    } else {
        addLog($debug, $log, 'Wrong GET parameter ps = '.$ps, 'error');
    }
    Payplug::redirectForVersion($redirect_url_error);
}
...
//Treatment
addLog($debug, $log, 'Cart ID : '.(int)$cart_id, 'info');
...
if (!Validate::isloadedObject($cart)) {
    ...
    if (!$pay_id = $payplug->getPaymentByCart((int)$cart_id)) {
        addLog($debug, $log, 'Payment is not stored or is already consumed.', 'error');
        ...
        addLog($debug, $log, 'Payment is not consumed yet.', 'info');
        try {
            $payment = \Payplug\Payment::retrieve($pay_id);
            addLog($debug, $log, 'Retrieving payment...', 'info');
            if ($payment->failure) {
                addLog($debug, $log, 'Payment failure : '.$payment->failure->message, 'error');
                Payplug::redirectForVersion($redirect_url_error);
            }
            $is_paid = $payment->is_paid;
        } catch (Exception $e) {
            addLog($debug, $log, 'Payment cannot be retrieved.', 'error');
            Payplug::redirectForVersion($redirect_url_error);
        }
    }

    if ($payment->save_card == 1 || ($payment->card->id != '' && $payment->hosted_payment != '')) {
        addLog($debug, $log, 'Saving card...', 'info');
        $res_payplug_card = $payplug->saveCard($payment);

        if (!$res_payplug_card) {
            addLog($debug, $log, 'Card cannot be saved.', 'error');
        }
    }

    addLog($debug, $log, 'Deleting stored payment.', 'info');
    if (!$payplug->deletePayment($payment->id, (int)$cart_id)) {
        addLog($debug, $log, 'Stored payment cannot be deleted.', 'error');
    } else {
        addLog($debug, $log, 'Stored payment successfully deleted.', 'info');
    }

    $customer = new Customer((int)$cart->id_customer);
    if (!Validate::isLoadedObject($customer)) {
        addLog($debug, $log, 'Customer cannot be loaded.', 'error');
        Payplug::redirectForVersion($redirect_url_error);
    }
    ...
    addLog($debug, $log, 'Total : '.$total, 'info');
    ...
    if ($id_order) {
        ...
        addLog($debug, $log, 'Order state will be :'.$order_state, 'info');
        ...
        if (!$validateOrder_result) {
            addLog($debug, $log, 'Order not validated', 'error');
            ...
            addLog($debug, $log, 'Order validated', 'info');
        }

        if (version_compare(_PS_VERSION_, '1.5', '<')) {
            if (!$payplug->addPayplugOrderPayment($id_order, $payment->id)) {
                addLog($debug, $log, 'Unable to create order payment.', 'error');
            }
        }
    ...
    addLog($debug, $log, 'Redirecting to :'.$link_redirect, 'info');
    Payplug::redirectForVersion($link_redirect);
}
