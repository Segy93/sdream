...
class Usercentrics extends Module
{
    ...
    public $register_link = array(
        ...
        'fr' => 'https://usercentrics.com/pricing/prestashop-special-fr/?partnerid=18930',
        'es' => 'https://usercentrics.com/pricing/prestashop-special-es/?partnerid=18930',
    );
    public $password_link = array(
        'de' => 'https://usercentrics.silbersaiten.de/de/ucpassword',
        'en' => 'https://usercentrics.silbersaiten.de/en/ucpassword',
        'fr' => 'https://usercentrics.silbersaiten.de/fr/ucpassword',
        'es' => 'https://usercentrics.silbersaiten.de/es/ucpassword',
    );
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Usercentrics Cookie Manager');
        $this->description = $this->l('Usercentrics Cookie Manager');
    ...
    public function hookDisplayBackofficeHeader()
    {
        if (Tools::getValue('configure') == $this->name) {
            Media::addJsDef(
                array(
                    'business_coming_soon' => $this->l('Business coming soon')
                )
            );
    ...
    public function displayGeneralSettings()
    {
        ...
        $fields_form = array();
        $password_link = isset($this->password_link[$this->context->language->iso_code]) ? $this->password_link[$this->context->language->iso_code] : $this->password_link['en'];
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'desc' => $this->l('Leave this field blank if there\'s no change.') .
                            ' <a href="' . $password_link . '" class="link " target="_blank">' . $this->l('Forgot password?') . '</a>'
                    ),
                    array(
                        'type' => 'free',
        ...
        $helper->fields_value['password_change'] = $this->displayCreateAccount();
    ...
    public function displayUCSettingSettings($setting_id)
    {
        ...
        if ($setting['en'] != false && isset($setting['en'])) {
            ...
            $values['reshowBanner'] = Tools::getValue('reshowBanner', is_null($language_setting['reshowBanner'])?0:1);
            $values['reshowBanner_value'] = (Tools::getValue('reshowBanner', $language_setting['reshowBanner']) > 0)?Tools::getValue('reshowBanner_value', $language_setting['reshowBanner']):'';
...
//                            'class' => 't',
                            ...
                            'desc' => $this->l('This resurfaces the CMP and refreshes the users consent choice after the selected time period.')
                        ),
                        array(
                            'type' => 'text',
                            'label' => $this->l(''),
                            'class' => 'reshowbanner_value fixed-width-xl',
                            'disabled' => ((Tools::getValue('reshowBanner', is_null($language_setting['reshowBanner'])?0:1)) > 0?'':'disabled'),
                            'name' => 'reshowBanner_value',
                            'placeholder' => $this->l('Time period in months (e.g. 12)')
                        ),
                        array(
                            'type' => 'color',
...
//                            'disabled' => false,
    ...
    private function displayCreateAccount()
    {
    ...
    private function postProcess()
    {
        ...
        if (Tools::getIsset('saveUCSetting')) {
            ...
            if ((Tools::getValue('reshowBanner', 0) == 1) && (!Validate::isUnsignedInt(Tools::getValue('reshowBanner_value')) || Tools::getValue('reshowBanner_value') == 0)) {
                $this->module_post_errors[] = $this->l('Time period for reshowing banner is invalid');
            }
            if (!count($this->module_post_errors)) {
                ...
                $data = array(
                    ...
                    'reshowBanner' => (Tools::getValue('reshowBanner', 0) == 1)?Tools::getValue('reshowBanner_value'):'',
