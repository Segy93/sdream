...
var usercentrics_configure = {
    init: function () {
        ...
        $('.uc_privacy_button img').css('background-color', $('input[name="color"]').val());
        ...
        $('input[type="radio"]').on('click', function() {
            $('input[name="'+$(this).attr('name')+'"]').each(function() {
                if ($(this).is(':checked')) {
                    ...
                    if ($(this).attr('name') == 'reshowBanner' && $(this).attr('id') == 'reshowBanner_on') {
                        $('input[name="reshowBanner_value"]').attr('disabled', false);
                    } else {
                        $('input[name="reshowBanner_value"]').attr('disabled', 'disabled');
                    }
                } else {
                    $('p.help-block[for="' + $(this).attr('id') + '"]').slideUp(400);
                    $('.show_for_' + $(this).attr('id')).closest('.form-group').slideUp(400);
        ...
        $('#technology').select2({
            debug: true,
            placeholder: $('#technology').data('placeholder'),
    ...
    setBusinessComingMessageToDisabledFields: function(){
        ...
        forError.each(function(){
            $(this).closest('.form-group').find('label.control-label').append('<p class="business_coming_msg badge badge-info">' + business_coming_soon + '</p>');
        });
    }
}
