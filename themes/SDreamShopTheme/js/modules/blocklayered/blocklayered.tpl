		...
		<form action="#" id="layered_form">
			<div>
				...
				{foreach from=$filters item=filter}
					...
					{/if}
					{if $filter.id_key == 1}
  <span class="layered_subtitle">Visina</span>
    <div id="vrvi" style="display:none"></div>
  <div class="yourslider_value"></div>
    <div id="crni23" style="width:91%; margin-left:12px; margin-bottom:21px;"> </div>
  <div class="layered_slider yourslider"></div>
  <script type="text/javascript">
    
    var filter = {$filter|json_encode};
	console.log(filter.values);
	console.log(Object.keys(filter.values).length);

	var mikrokozma = Object.keys(filter.values);
	console.log(mikrokozma);
	var cekic= 100/mikrokozma.length;
	console.log(cekic);
	
	for (j=0;j<mikrokozma.length;j++){
			
			console.log( filter.values[mikrokozma[j]].name);
			
		 var node = document.createElement("div");
  
     node.setAttribute("id", "range1");
	 node.setAttribute("style", 'float:left; width:'+ cekic +'%;text-align:right;');
    ...
    $('.layered_slider.yourslider').slider({
      ...
      slide: function( event, ui ) {
		...
		console.log(zuki);
		document.getElementById('vrvi').innerHTML=zuki;
